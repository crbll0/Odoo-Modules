...
class cjc_invoice_wizard(models.TransientModel):
    ...
    line_ids = fields.One2many("cjc_request.wizard_line", "invoice_id", "Productos", select=False, required=True,
                               ondelete='cascade')
    ...
    def create_purchase(self):
        ...
        limit = current_model.monto_solicitado
        cantidad = current_model.total
        temp = 0

        for cnt in self.line_ids:
            temp += cnt.amount

        if sum([temp, cantidad]) > limit:
            raise Warning('Los montos de las Facturas a registrar supera el monto el monto solicitado')
        ...
        lines_vals = {u'account_id': current_model.journal_id.default_debit_account_id.id,
                      ...
                      u'ref': inv.number,
                      u'sequence': 0,
                      u'statement_id': current_model.caja_id.id,
                      # u'type': u'supplier',
                      # u'voucher_id': False,
                      u"invoice_id": inv.id,
        ...
        cjc_lines = {
                u'fecha': inv.date_invoice,
                u'comunicacion': self.concept,
                u'referencia': inv.number,
                u'partner_id': self.partner_id.id or self.journal_id.special_partner.id,
                u'importe': inv.amount_total,
                u'cjc_id': current_model.id,
        }

        self.pool.get('cjc_request.cjc_line').create(self.env.cr, self.env.uid, cjc_lines, context=self.env.context)

        lineas_cjc = self.env['cjc_request.cjc_line'].search(
                    [('cjc_id', '=', current_model.id)])
        invoice = self.env['account.invoice']

        timporte = 0
        for obj in lineas_cjc:
            cnt = invoice.search([('move_id', '=', obj.referencia)]).amount_untaxed
            timporte += cnt

        cjc = self.env['cjc_request.cjc_request'].browse(current_model.id)
        cjc.total = timporte

        self.pool.get('account.bank.statement.line').create(self.env.cr, self.env.uid, lines_vals, context=self.env.context)

        self.pool.get('account.bank.statement')._end_balance(
            self.env.cr, self.env.uid, current_model.caja_id.id, '', '')

        return {'type': 'ir.actions.act_window_close'}


class cjc_invoice_line_wizard(models.TransientModel):
    _name = "cjc_request.wizard_line"
    ...
    invoice_id = fields.Many2one("cjc_request.wizard", "Factura", ondelete='cascade', select=True)
